...
GROUPS = {
    "大码性感透视款": ["B0CL4WHHJC", "B0D628WFD5", "B0G33JQPYP", "B0GJS1HTLC", "B0GQ3K1XRW"],
}
SOURCE_DIRS = [
    Path("G:/伊羽霓/AI-开发/20260629大码性感透视"),
]
OUT_DIR = Path("outputs/20260704_plus_size_sexy_sheer_voc_product")
SOURCE_COPY = OUT_DIR / "source_inputs"
OUT_FILE = OUT_DIR / f"VOC_PRODUCT_大码性感透视款产品专家分析_{datetime.now().strftime('%H%M%S')}.xlsx"
...
TAXONOMY = [
    ...
    },
    {
        ...
        "neg": ["see through", "sheer", "thin", "too thin", "show through", "pad through", "pads visible", "nipple", "nipples", "nipple show", "nipples show", "nipple visible", "nipples visible", "see my nipples", "visible", "white tank"],
    ...
    },
    {
        ...
        "neg": ["hot", "sweaty", "too thick", "not breathable"],
    ...
    },
    {
        ...
        "neg": ["too thin", "nipple", "nipples", "nipple show", "nipples show", "nipple visible", "nipples visible", "see my nipples", "show through", "see through", "no padding", "needs padding"],
        "mechanism": "薄杯无衬带来轻薄透气和自然胸型，但杯布、遮点层和面料密度决定是否露点。",
        "kano": "期望型",
    },
    {
        "label": "Minimizer显小/大胸视觉收拢",
        "pos": ["minimizer", "minimize", "minimizes", "smaller", "reduce", "overlarge bust", "heavy breast"],
        "neg": ["not a minimizer", "doesn't minimize", "does not minimize", "not minimizing", "too pointy"],
        "mechanism": "显小效果来自杯型分散、侧收、中心前高和上胸压平控制，不能只依赖薄杯。",
        "kano": "期望型",
    },
    {
        "label": "钢圈稳定/不戳不压",
        "pos": ["underwire", "wire", "wires", "doesn't poke", "does not poke", "no poking"],
        "neg": ["underwire pokes", "wire pokes", "poking", "wire hurts", "wire digs", "underwire uncomfortable"],
        "mechanism": "钢圈弧度、钢圈长度、鸡心高度和杯底围度决定大码用户的支撑与压迫感。",
        "kano": "基本型",
    },
    {
        "label": "背部/侧翼包裹平滑",
        "pos": ["wide back", "back coverage", "side support", "side coverage", "smooth back", "back smoothing"],
        "neg": ["back fat", "side bulge", "side boob", "rolls", "bulge", "muffin top"],
        "mechanism": "侧翼高度、背带宽度、弹力网布和腋下杯边共同决定副乳和背部平滑。",
        "kano": "期望型",
    },
    {
        "label": "透视网纱/性感外观",
        "pos": ["sexy", "sheer", "mesh", "see-through", "see through", "lace", "leopard", "cute", "beautiful"],
        "neg": ["too sheer", "too see through", "too sexy", "cheap looking", "scratchy lace", "itchy lace"],
        "mechanism": "透视感、网纱密度、蕾丝触感和杯型边线共同决定性感但不廉价的视觉表达。",
        "kano": "魅力型",
    },
    {
        "label": "Plunge/Balconette领型聚拢",
        "pos": ["plunge", "balconette", "demi", "low cut", "cleavage", "shape", "nice shape"],
        "neg": ["gaping", "spill", "spillage", "falls out", "too low", "not enough coverage"],
        "mechanism": "低鸡心、杯口弧度、侧推结构和杯容决定性感领型是否还能稳住胸部。",
        "kano": "期望型",
    },
]
...
NEGATIVE_LABELS = {
    ...
    "钢圈稳定/不戳不压": "钢圈戳肉/压迫疼痛",
    "背部/侧翼包裹平滑": "副乳/背部勒肉外溢",
    "透视网纱/性感外观": "过透/蕾丝刺痒/廉价感",
    "Plunge/Balconette领型聚拢": "低胸走光/杯口空杯外溢",
}
...
POSITIVE_KEYWORD_LABELS = [
    ...
    (["minimizer", "minimize", "minimizes", "overlarge bust", "heavy breast"], "大胸显小/视觉收拢"),
    (["unlined", "thin cup", "thin cups", "no padding", "unpadded"], "薄杯无衬/轻薄自然"),
    (["sexy", "sheer", "mesh", "see-through", "see through", "lace", "leopard"], "透视性感/网纱外观好看"),
    (["plunge", "balconette", "demi", "low cut", "cleavage"], "低鸡心/半杯聚拢好看"),
    (["underwire", "doesn't poke", "does not poke", "no poking"], "钢圈稳定不戳"),
...
NEGATIVE_KEYWORD_LABELS = [
    ...
    (["too thin", "nipple", "nipples", "nipple show", "nipples show", "nipple visible", "nipples visible", "see my nipples", "show through", "see through", "needs padding"], "薄杯露点/遮点不足"),
    (["too sheer", "too see through", "too sexy", "cheap looking", "scratchy lace", "itchy lace"], "过透/蕾丝刺痒/廉价感"),
    (["gaping", "spill", "spillage", "falls out", "too low", "not enough coverage"], "杯口空杯/低胸外溢"),
    ...
    (["see through", "sheer", "thin", "too thin", "show through"], "面料薄/透"),
    (["nipple", "nipples", "nipple show", "nipples show", "nipple visible", "nipples visible", "see my nipples", "visible", "pads visible", "pad through"], "露点/杯垫显形"),
...
def sheet_headers(wb, sheet):
    ws = wb[sheet]
    return [str(c.value or "").strip() for c in ws[1]]


def find_review_sheet(wb):
    if "评论分析" in wb.sheetnames:
        return "评论分析"
    for sheet in wb.sheetnames:
        headers = set(sheet_headers(wb, sheet))
        if {"型号", "标题", "内容", "星级"}.issubset(headers):
            return sheet
    return ""


def find_order_sheet(wb):
    if "订单量分析" in wb.sheetnames:
        return "订单量分析"
    for sheet in wb.sheetnames:
        headers = set(sheet_headers(wb, sheet))
        if {"Color (颜色)", "Size (尺寸)", "近30天订单量"}.issubset(headers):
            return sheet
    return ""


def extract_order_images(wb, asin, order_sheet="订单量分析"):
    if not order_sheet or order_sheet not in wb.sheetnames:
        return {}
    ws = wb[order_sheet]
...
def infer_body_params(data):
    ...
    return f"核心订单码数：{sizes}；评论尺码：{review_sizes}；体型信息以胸围杯码订单与评论型号为主。"
...
def persona_text(data, age_group):
    ...
    pref = []
    for label in ["薄杯无衬/自然胸型", "Minimizer显小/大胸视觉收拢", "支撑承托/DD+适配", "钢圈稳定/不戳不压", "背部/侧翼包裹平滑", "贴肤舒适/无勒痕无刺激"]:
    ...
    return {
        ...
        "商品标题": data["title"],
        "年龄段": age_group,
        "用户标签": "性感透视文胸用户 / 追求薄杯网纱 / 兼顾支撑与胸型",
        "用户画像她是什么样的人": f"{age_group} 女性，想要一件有性感透视外观、薄杯轻盈、又能支撑和修饰胸型的文胸。",
        "核心关注的点": top_good,
...
def group_persona_text(group, items):
    ...
    return (
        f"1、年龄段，用户标签：{group}；性感透视文胸用户 / 追求薄杯网纱 / 兼顾支撑与胸型。\n"
        f"用户画像她是什么样的一个人？{group} 女性，想要透视网纱的性感外观，同时不能牺牲支撑、舒适和杯型稳定。核心关注：{top_good}\n"
...
def analyze_asin(asin, path):
    ...
    review_sheet = find_review_sheet(wb)
    order_sheet = find_order_sheet(wb)
    reviews = read_rows(wb, review_sheet) if review_sheet else []
    orders = read_rows(wb, order_sheet) if order_sheet else []
    row_images = extract_order_images(wb, asin, order_sheet)
    title = ""
    brand = ""
    if orders:
