...
GROUPS = {
    "大码薄杯款": ["B0F94KNDQT", "B0G8HHC7SN", "B0GQ3RYX1F", "B0F5PP9MPT", "B0CP3F7PNL"],
}
SOURCE_DIRS = [
    Path("G:/伊羽霓/AI-开发/20260617大码薄杯款"),
]
...
def persona_text(data, age_group):
    ...
    return {
        ...
        "商品标题": data["title"],
        "年龄段": age_group,
        "用户标签": "大码薄杯文胸用户 / 重视支撑显小 / 追求舒适不厚重",
        "用户画像她是什么样的人": f"{age_group} 女性，胸部较丰满，想要一件薄杯不厚重、能支撑收拢、日常穿不勒不戳的文胸。",
        "核心关注的点": top_good,
...
def group_persona_text(group, items):
    ...
    return (
        f"1、年龄段，用户标签：{group}；大码薄杯文胸用户 / 重视支撑显小 / 追求舒适不厚重。\n"
        f"用户画像她是什么样的一个人？{group} 女性，胸部较丰满，想要薄杯无厚垫、支撑稳定、显小收拢且全天舒适的文胸。核心关注：{top_good}\n"
